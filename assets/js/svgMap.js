...
svgMap.prototype.applyData = function (data) {
  ...
  //I added this block for my blog.
  //Eren
  if(data.data.isVisited != null){
    var silver_flag = 1;
    var time = new Date().toLocaleString('en-US', { hour: 'numeric', hour12: true });
    if(time.includes("AM")){
      let control = time[0] + time[1];
      const hour = parseInt(control);
      //7AM - 11AM
      if(hour >= 7 && hour <= 11){
        document.getElementsByClassName('svgMap-map-image')[0].style.backgroundColor= '#a3cffa';
      }
      //12AM - 6AM
      else{
        document.getElementsByClassName('svgMap-map-image')[0].style.backgroundColor= '#143552';
        silver_flag = false;
      }
    }
    else{
      ...
      //12PM - 5PM
      if(hour <= 5){
        document.getElementsByClassName('svgMap-map-image')[0].style.backgroundColor= '#76baff';
      }
      //6PM - 11PM
      else{
        document.getElementsByClassName('svgMap-map-image')[0].style.backgroundColor= '#143552';
    ...
    Object.keys(this.countries).forEach(function (countryID) {
      ...
      if (!data.values[countryID]) {      
        element.setAttribute('fill', '#4f5a64');
        if(silver_flag){
...
svgMap.prototype.getTooltipContent = function (countryID) {
  ...
  if (!this.options.data.values[countryID]) {
    ...
    Object.keys(this.options.data.data).forEach(function (key) {
      ...
      value = item.format ? item.format.replace('{0}', '<a href="#index.html"><span>' + value + '</span></a>') : '<a href="#index.html"><span>' + value + '</span></a>';
...
svgMap.prototype.mapPaths = {
  ...
  "UA": {
    "d": "M1138.5,241l-4.8.5-1.5-.3-1,1.4-1.8-.2h0l-4.1.3-1.2,1.4.2,3.1-2-.6-4.3.3-1.5-1.5-1.6,1.1-2-.9h-3.8l-5.6-1.5-5-.5-3.7.2-2.4,1.6-2.2.3,3.1,5.3-.3,1.8-2.3.7-3.8,5.1,1.6,2.8-1.1-.4-1.1,1.7-.7,2.5,2.9,1.7.6,1.6,1.9-1.3,3.2.7h3.2l2.4,1.5,1.6-.9,3.6-.6,1-1.5h2.1l1.1-.9,3.2-.6,3.9,1.9,2,.3,2.5,1.6v2.1l1.9,1.1,1.1,2.6,2,1.5-.2,1,1,.6-1.2.5-3-.2-.6-.9-1,.5.5,1.1-1.1,2-.5,2.1-1.2.7,2.4,1.1,2.2-1,2.4,1.1,3.3-4.6,1.3-3.4,4.5-.8.7,2.4,8,1.5,1.7,1.4,7.4,1.3-1-3.9,3.9-2.3,4.6-.3,3-2,3.9-.5-.4-2.8,2.2-1.7,4.7-.5.3-2.1-1.8-3.4,1.3-3.2-.4-1.9-7.6-2h-2.9l-3.6-2.9-3.5,1-6.6-2.2-.2-1.2-2.2-2.7-4-.2-.7-1.9.9-1.3-3.8-3.4Z"
  },
  "AE": {
